...
public class StopManager implements Iterable<Stop> {
    ...
    public Stop getStopWithId(int id) {
        for(Integer key : stopMap.keySet()){
            if (key== id){
                Stop value = stopMap.get(key);
                return value;
            }
        }
        Stop s=new Stop(id,"", new LatLon(49.25,-123.12)  );
        stopMap.put(id,s);
        return s;
    }
     ...
     */
    public void clearSelectedStop() {selected=null;
     ...
     */
    public void clearStops() { stopMap.clear();
    ...
    public Stop findNearestTo(LatLon pt) {
        ...
        for(Integer key : stopMap.keySet()){
            Stop value = stopMap.get(key);
            double area= SphericalGeometry.distanceBetween(value.getLocn(),pt);
            if (area < distance) {
                closest = value;
                distance = area;
            }

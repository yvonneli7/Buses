...
public class MapDisplayFragment extends Fragment implements MapEventsReceiver, IMyLocationConsumer {
    ...
    private void plotRoutes() {
        ...
        List <GeoPoint> listgeo = new ArrayList<>();




        if (s != null) {
            for (Route path : s.getRoutes()) {

                int colour=  busRouteLegendOverlay.add(path.getNumber());

                if (path != null) {


                    for (RoutePattern rp : path.getPatterns()) {
                        if (rp != null) {
                            ...
                            for (int index = 0; index < rp.getPath().size()-1; index++) {
                                if(!(rp.getPath().get(index+1) == null) ){


                                    LatLon l = rp.getPath().get(index);
                                    LatLon newl = rp.getPath().get(index + 1);


                                    if (Geometry.rectangleIntersectsLine(northWest, southEast, l, newl)) {
                                        GeoPoint x = new GeoPoint(l.getLatitude(), l.getLongitude());
                                        GeoPoint y = new GeoPoint(newl.getLatitude(), newl.getLongitude());
                                        listgeo.add(x);
                                        listgeo.add(y);
                                        pl.setPoints(listgeo);
                                        pl.setColor(colour);
...
//                + southeast.getLatitude() + " " + southeast.getLongitude());
    ...
    private void markStops() {
        ...
        for (Stop stop : StopManager.getInstance()) {
            if (Geometry.rectangleContainsPoint(northWest, southEast, stop.getLocn())) {
                ...
                setMarker(stop, mark);
                mark.setRelatedObject(stop);
                mark.setInfoWindow(stopInfoWindow);
                mark.setIcon(stopIconDrawable);
                mark.setTitle(stop.getName() + stop.getNumber());
                stopClusterer.add(mark);

                Double lat = stop.getLocn().getLatitude();
                Double lon = stop.getLocn().getLongitude();
    ...
    private void updateMarkerOfNearest(Stop nearest) {
        ...
        if ((nearest != null) && (nearestStopMarker != null)) {
            if(getMarker(nearest) != null) {
                if (!(getMarker(nearest).equals(nearestStopMarker))) {
    ...
    private void handleLocationChange(Location location) {
        ...
        LatLon current = new LatLon(lat, lon);
        Stop s = StopManager.getInstance().findNearestTo(current);
        updateMarkerOfNearest(s);
        locationListener.onLocationChanged(s, current);
    }

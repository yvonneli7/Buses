...
public class RouteMapParser {
    ...
    private void parseOnePattern(String str) {
        ...
        List<LatLon> locns=new ArrayList<>();
        String[] locnslist= str.split(";");


        String routeNumber =  locnslist[0].substring(1, str.indexOf("-"));
        String PatternName= locnslist[0].substring(str.indexOf("-")+1);



        for(int i=1;i<locnslist.length; i += 2){
            locns.add(new LatLon(Double.parseDouble(locnslist[i]), Double.parseDouble(locnslist[i+1])) );
        }

        storeRouteMap(routeNumber, PatternName, locns);
